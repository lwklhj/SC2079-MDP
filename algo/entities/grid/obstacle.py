    ...
    def __init__(self, x, y, direction, index):
        ...
        if (x - 5) % 10 != 0 or (y - 5) % 10 != 0:
            raise AssertionError(
                "Obstacle center coordinates must be multiples of 10 with offset 5!")

        # Translate given coordinates to be in PyGame coordinates.
        self.pos = Position(x * settings.SCALING_FACTOR,
                            y * settings.SCALING_FACTOR, direction)

        # Arrow to draw at the target coordinate.
        self.target_image = pygame.transform.scale(pygame.image.load("algo/entities/assets/target-arrow.png"),
                                                   (50, 50))
