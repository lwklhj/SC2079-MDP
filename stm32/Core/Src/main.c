...
void StartInfraredTask(void *argument)
{
  ...
  for(;;)
  {
    //osDelay(1);
	  ...
	  float voltage = value / 4095 * 3.3;

	  // Remove once we get sample points
	  uint8_t msg[] = "IR sensor voltage: " + voltage;
	  HAL_UART_Transmit(&huart3, msg, sizeof(msg), HAL_MAX_DELAY);

	  // Calculate distance
	  uint8_t distance = 29.998 * pow(voltage, -1.173);
	  //uint8_t msg[] = "" + distance;
	  //HAL_UART_Transmit(&huart3, msg, sizeof(msg), HAL_MAX_DELAY);
  }
  /* USER CODE END 5 */
}
